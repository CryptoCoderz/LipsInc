...
static const int MAX_OUTBOUND_CONNECTIONS = DEFAULT_MAX_OUTBOUND_CONNECTIONS;
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "LipsInc " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. LipsInc is probably already running."), addrBind.ToString());
        else
