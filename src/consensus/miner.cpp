...
/* Test for GCC < 6.3.0 */
    ...
    int64_t nFees = 0;
    {
        ...
        if (!fProofOfStake){
            ...
            if(pindexBest->GetBlockTime() > 0){ // ON
                if(pindexBest->GetBlockTime() > 0){ // ON
                    ...
                    CAmount devopsPayment = blockReward;
                    ...
                    ExtractDestination(do_payee, address1);
                    CBitcoinAddress address2(address1);
                    LogPrintf("CreateNewBlock(): Devops payment %lld to %s\n",
                    devopsPayment, address2.ToString().c_str());
                }
            } //
        }
