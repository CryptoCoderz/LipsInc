...
{
    // LipsInc: check prefix
    if(uri.scheme() != QString("LipsInc"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::LIPS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert LipsInc:// to LipsInc:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("LipsInc://", Qt::CaseInsensitive))
    {
        uri.replace(0, 12, "LipsInc:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LipsInc.lnk";
}
...
{
    return GetAutostartDir() / "LipsInc.desktop";
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the LipsInc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add LipsInc app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LipsInc-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  LipsInc-qt [" + tr("command-line options") + "]                     " + "\n";
