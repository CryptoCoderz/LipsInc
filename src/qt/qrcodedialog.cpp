...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // start the display unit, uses the default ("LIPS")
    updateDisplayUnit();
}
...
{
    QString ret = QString("LipsInc:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LIPS unit input in lnReqAmount, we generate the URI with LIPS as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LIPS, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
