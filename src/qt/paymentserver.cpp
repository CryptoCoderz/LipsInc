...
const int LIPSINC_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString LIPSINC_IPC_PREFIX("LipsInc:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(LIPSINC_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(LIPSINC_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(LIPSINC_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start LipsInc: click-to-pay handler");
    else
