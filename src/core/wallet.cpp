...
int64_t GetStakeCombineThreshold() { return GetArg("-stakethreshold", 10000) * COIN; }
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s LIPS %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s LIPS %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (Params().NetworkID() == CChainParams::MAIN) {
        devopaddress = CBitcoinAddress("LN8kaMgUruQqXWv9ERohjNwzpmiWai9ZoL");
    ...
    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > 9993058800 ){// OFF (NOT TOGGLED)
    ...
    if(bDevOpsPayment) {
        ...
        {
            //spork
            if(pindexBest->GetBlockTime() > 0) { // ON  (TOGGLED)
    ...
    if(hasdevopsPay)
    {
        if(txNew.vout.size() == 3){ // only 1 stake output, was not split, plus a devops payment
            txNew.vout[devoppay-1].nValue = devopsPayment;
            blockValue -= devopsPayment;
            txNew.vout[1].nValue = blockValue;
        } else {// We should never get here
            return error("CreateCoinStake : fatal error creating stake outputs, expected 3, got %d", txNew.vout.size());
        }
    } else {// We should never get here
         return error("CreateCoinStake : failed to pay devops");
    }
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s LIPS %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s LIPS %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
